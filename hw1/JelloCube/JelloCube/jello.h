...
struct world
{
	...
	struct Vector3 * forceField; // pointer to the array of values of the force field
	struct Vector3 p[8][8][8]; // position of the 512 control points
	struct Vector3 v[8][8][8]; // velocities of the 512 control points
};
...
#define DOTPRODUCT(v1, v2, dst)\
\
	dst = v1.x * v2.x + v1.y * v2.y + v1.z * v2.z;\

#define LENGTH(src, dst)\
\
...
#define P_DIVIDE(src, divider, dst)\
\
	(dst).x = (src).x / divider;\
	(dst).y = (src).y / divider;\
	(dst).z = (src).z / divider;
