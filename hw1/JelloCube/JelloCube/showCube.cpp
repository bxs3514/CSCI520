...
{
	int i, j, k, ip, jp, kp;
	Vector3 r1, r2, r3; // aux variables

	/* normals buffer and counter for Gourad shading*/
	struct Vector3 normal[8][8];
	int counter[8][8];
...
#define NODE(face,i,j) (*((struct Vector3 * )(jello->p) + pointMap((face),(i),(j))))
...
void showInclinePlane()
{
	if (jello.incPlanePresent == 1)
	{
		...
		for (int i = -2; i <= 2; i += 4)
		{
			for (int j = -2; j <= 2; j += 4)
			{
				double x = (-jello.d - jello.b * (double)i - jello.c * (double)j) / jello.a;
				double y = (-jello.d - jello.a * (double)i - jello.c * (double)j) / jello.b;
				double z = (-jello.d - jello.a * (double)i - jello.b * (double)j) / jello.c;
				Vector3 v;
				bool multi = false;

				if (abs(x) <= boundingBoxRange)
				{
					v = { x, (double)i, (double)j };
					for (int i = 0; i < count; ++i)
					{
						if (v.x == corners[i].x && v.y == corners[i].y && v.z == corners[i].z)
						{
							multi = true;
							break;
						}
					}
					if (!multi) corners[count++] = v;
				}
				if (abs(y) <= boundingBoxRange)
				{
					v = { (double)i, y, (double)j };
					for (int i = 0; i < count; ++i)
					{
						if (v.x == corners[i].x && v.y == corners[i].y && v.z == corners[i].z)
						{
				...
				if (abs(z) <= boundingBoxRange)
				{
					v = { (double)i, (double)j, z };
					for (int i = 0; i < count; ++i)
					{
						if (v.x == corners[i].x && v.y == corners[i].y && v.z == corners[i].z)
						{
